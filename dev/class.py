...
class MzmlClass:
    def __init__(self, mzml_scans):
        self.mzml_scans = mzml_scans
    
    def peak_pick(self, input_mz, error=20, enable_score=False, peak_thres=0.01,
        ...
        '''
        if not rt:
            rt = [i.scan_time[0] for i in self.mzml_scans]
        intensity = msm.ms_chromatogram_list(self.mzml_scans, input_mz, error)
